...
import com.blockchain.server.cmc.common.util.BtcBlockRedisUtils;
...
//            }
    ...
    private void parseBtcTransfer(JSONObject transferBtc, String txId) {
        //交易金额，正数表示该交易增加钱包余额，负数表示该交易减少钱包余额  同一个节点转账这个是0，不同节点转账这个不是0

        //比如你现在链接的是A节点转出的时候查询是负数 那么到B节点去查询的时候就是正数
