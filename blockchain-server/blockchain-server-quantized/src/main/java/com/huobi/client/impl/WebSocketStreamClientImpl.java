...
public class WebSocketStreamClientImpl implements SubscriptionClient {
  ...
  public void unSubscribeSymbol(String symbol) {
    ...
    Iterator<WebSocketConnection> dd = connections.iterator();
    while (dd.hasNext()) {
      WebSocketConnection connection = dd.next();
      WebsocketRequest websocketRequest = connection.getWebsocketRequest();
      String _name = "["+symbol+"]";
      if(websocketRequest != null && (websocketRequest.name.indexOf(_name) != -1)){
        watchDog.onClosedNormally(connection);
        connection.close();
        dd.remove();
      }
